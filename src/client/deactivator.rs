...
"
        ...
        ";
let data_to_sign = "".join(vec![EntryType::Deactivation.value, ENTRY_SCHEMA_V100, self.signing_key.full_id(self.did.id)]);
let signature = self.signing_key.sign(hashlib.sha256(data_to_sign.encode("utf-8")).digest());
let ext_ids = vec![EntryType::Deactivation.value.encode("utf-8"), ENTRY_SCHEMA_V100::encode("utf-8"), self.signing_key.full_id(self.did.id).encode("utf-8"), signature];
return [("ext_ids", ext_ids), ("content", b"")].iter().cloned().collect::<HashMap<_,_>>();
}
