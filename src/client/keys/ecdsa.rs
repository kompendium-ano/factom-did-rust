...
return "<{}.{}(public_key={}, private_key=({}))>".format(self.__module__, type_(self).__name__, base58.b58encode(self.verifying_key.to_string()).decode("utf-8"), if self.signing_key != None { "hidden" } else { "not set" });
}
fn public_key<RT>(&self) -> RT {
return self.verifying_key.to_string();
}
fn private_key<RT>(&self) -> RT {
return if self.signing_key != None { self.signing_key.to_string() } else { None };
}
...
let except!((AssertionError, ValueError)) = { //unsupported
raise!(ValueError("Invalid ECDSA private key. Must be a 32-byte secret exponent.")); //unsupported
};
self.verifying_key = self.signing_key.get_verifying_key();
...
let except!((AssertionError, ValueError)) = { //unsupported
raise!(ValueError("Invalid ECDSA public key. Must be a 64-byte encoded SECP256k1 curve point.")); //unsupported
};
} else {
let try_dummy = { //unsupported
self.signing_key = ecdsa.SigningKey.from_string(private_key, SECP256k1);
