...
use factom_did::client::validators::{validate_alias, validate_did, validate_key_type, validate_priority_requirement};
...
raise!(NotImplementedError("Unsupported signature type: {}".format(self.key_type.value))); //unsupported
}
}
}
}
...
return (self.alias, self.key_type, self.controller, self.priority_requirement, self.public_key, self.private_key) == (other.alias, other.key_type, other.controller, other.priority_requirement, other.public_key, other.private_key);
}
...
return if hash_f != None { self.underlying.sign(message, hash_f) } else { self.underlying.sign(message) };
}
fn verify<T0, T1, T2, RT>(&self, message: T0, signature: T1, hash_f: T2) -> RT {
return if hash_f != None { self.underlying.verify(message, signature, hash_f) } else { self.underlying.verify(message, signature) };
}
...
"
        ...
        ";
if version == ENTRY_SCHEMA_V100 {
return AbstractDIDKey(entry_dict.get("id", "").split("#")[-1], KeyType::from_str(entry_dict.get("type")), entry_dict.get("controller"), entry_dict.get("priorityRequirement"), if entry_dict.iter().any(|&x| x == "publicKeyBase58") { base58.b58decode(entry_dict["publicKeyBase58"]) } else { entry_dict.get("publicKeyPem") });
} else {
raise!(NotImplementedError("Unknown schema version: {}".format(version))); //unsupported
}
}
...
fn _validate_key_input_params<T0, T1, T2, T3>(alias: T0, key_type: T1, controller: T2, priority_requirement: T3)  {
